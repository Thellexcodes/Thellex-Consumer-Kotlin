        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                if (token.isNullOrEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        CustomToast.show(this@IDVerificationAttestation, "Error", "User not authenticated")
                    }
                    return@launch
                }
                ...
                if (response.isSuccessful) {
                    ...
                    if (result != null) {
                        ...
                        withContext(Dispatchers.Main) {
                            startActivity(Intent(this@IDVerificationAttestation, KycSuccessActivity::class.java))
                            finish()
                        }
                    } else {
                        Log.e(TAG, "Verification success but result body is null")
                        withContext(Dispatchers.Main) {
                            CustomToast.show(this@IDVerificationAttestation, "Error", "Verification returned empty result")
                        }
                    }
                } else {
                    ...
                    withContext(Dispatchers.Main) {
                        ErrorHandler.handle(this@IDVerificationAttestation, "Error", userError)
                    }
                }
            } catch (e: Exception) {
