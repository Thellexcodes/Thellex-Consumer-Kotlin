...
import com.thellex.pay.databinding.ActivityRampSummaryBinding
...
class OffRampSummaryActivity : AppCompatActivity() {
    ...
    private fun setupTransactionSummary() = with(binding.offrampTransactionDetails) {
        ...
        rampBlockchainNetworkName.text = Helpers.formatNetworkName(viewModel.network.value).uppercase().orEmpty()
        ...
        viewModel.assetCode.value?.let { assetCode ->
            viewModel.currentRate.value?.sell?.let { rate ->
                ...
                val adjustedRate = rate * 0.98  // remove 2%
                rampExchangeRateValue.text = "${adjustedRate} $fiatCode/${assetCode.uppercase()}"
            }
        }
    }
